...
async fn test_randomized_fragmentation_and_window_size() {
    ...
    let server = start_socks5_server("127.0.0.1:0", outbound, false, opts).await.unwrap();
    ...
    req.extend_from_slice(&addr.ip().to_string().parse::<std::net::Ipv4Addr>().unwrap().octets());

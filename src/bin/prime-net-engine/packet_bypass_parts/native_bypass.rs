...
impl NativeBypassGuard {
    ...
    /// Build a guard from [`EvasionConfig`], merging user-defined profiles with defaults.
    pub fn start_with_config(cfg: &EvasionConfig) -> Self {
        let engine = TcpDesyncEngine::with_config(cfg);
        info!(
            target: "native_bypass",
            profiles = engine.profile_count(),
            "native bypass engine started (userspace-only mode)"
...
///
/// Uses default profiles. Prefer [`maybe_start_native_bypass_with_config`] when
/// an [`EvasionConfig`] is available to include user-defined profiles.
...
/// Try to start the native bypass engine with user-defined profiles from config.
///
/// Always succeeds (no external dependencies).
pub fn maybe_start_native_bypass_with_config(
    enabled: bool,
    cfg: &EvasionConfig,
) -> Option<NativeBypassGuard> {
    if !enabled {
        return None;
    }
    Some(NativeBypassGuard::start_with_config(cfg))
}

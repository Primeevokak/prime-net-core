...
pub async fn handle_socks4(
    ...
    if cmd != 0x01 { return Err(EngineError::Internal("unsupported socks4 cmd".to_owned())); }
    let mut p_buf = [0u8; 2]; tcp.read_exact(&mut p_buf).await?;
    let port = u16::from_be_bytes(p_buf);
    let mut ip_buf = [0u8; 4]; tcp.read_exact(&mut ip_buf).await?;
    let target_addr = TargetAddr::Ip(std::net::IpAddr::V4(ip_buf.into()));
    
    // Read user ID (and discard)
    loop {
        let mut b = [0u8; 1]; tcp.read_exact(&mut b).await?;
        if b[0] == 0 { break; }
    }

    let mut out = outbound.connect(TargetEndpoint { addr: target_addr.clone(), port }).await?;
    ...
    let (c2u, u2c) = relay_bidirectional(&mut tcp, &mut out, tuned.options, Vec::new(), Vec::new(), false).await?;

...
pub async fn handle_http_proxy(
    ...
    if method == "CONNECT" {
        ...
        let candidates = select_route_candidates(&relay_opts, &target_endpoint.addr, target_endpoint.port, &target_label, &cfg);
        
        tcp.write_all(b"HTTP/1.1 200 Connection Established\r\n\r\n").await?;
        handle_socks5_request_with_target(conn_id, tcp, peer, &client, &target_endpoint, outbound, cfg, relay_opts, None).await
    } else {
        Ok(())
    }
}

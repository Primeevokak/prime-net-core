...
pub fn init_telemetry_bus(cfg: Arc<EngineConfig>) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(event) = rx.recv().await {
            match event {
                TelemetryEvent::RouteSuccess { route_key, candidate } => {
                    route_scoring::record_route_success_sync(&route_key, &candidate, &cfg);
                }
                TelemetryEvent::RouteFailure { route_key, candidate, reason } => {
                ...
                TelemetryEvent::GlobalBypassFailure { candidate, reason } => {
                    route_scoring::record_global_bypass_profile_failure_sync(&candidate, reason, now_unix_secs(), &cfg);
                }
                TelemetryEvent::DestinationSuccess { destination, stage } => {
                    classifier_and_persistence::record_destination_success_sync(&destination, stage, &cfg);
                }
                TelemetryEvent::DestinationFailure { destination, signal, stage } => {
                    classifier_and_persistence::record_destination_failure_sync(&destination, signal, stage, &cfg);
                }
                TelemetryEvent::MlDecision { route_key, candidates, raced, canary, decision_id } => {
                    ml_shadow::begin_route_decision_event_sync(&route_key, &candidates, raced, canary, decision_id, &cfg);
                }
                TelemetryEvent::MlOutcome { decision_id, candidate, connect_ok, tls_ok_proxy, bytes_u2c, lifetime_ms, error_class } => {
                    ml_shadow::complete_route_outcome_event_sync(decision_id, candidate.as_ref(), connect_ok, tls_ok_proxy, bytes_u2c, lifetime_ms, &error_class, &cfg);
                }
            }

...
pub enum BlockingSignal { Reset, Timeout, EarlyClose, BrokenPipe, SuspiciousZeroReply, SilentDrop }
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum RouteKind { Direct, Bypass }
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum RouteIpFamily { V4, V6, Any }
impl RouteIpFamily { pub fn label(&self) -> &'static str { match self { Self::V4 => "v4", Self::V6 => "v6", Self::Any => "any" } } }
...
pub struct RouteWinner { pub route_id: String, pub updated_at_unix: u64 }
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct RouteHealth { pub successes: u64, pub failures: u64, pub consecutive_failures: u8, pub weak_until_unix: u64, pub last_success_unix: u64, pub last_failure_unix: u64 }
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
pub struct RouteMetrics { pub race_started: AtomicU64, pub race_skipped: AtomicU64, pub route_success_direct: AtomicU64, pub route_success_bypass: AtomicU64, pub route_failure_direct: AtomicU64, pub route_failure_bypass: AtomicU64 }
#[derive(Debug, Clone, Default)]
pub struct TunedRelay { pub options: RelayOptions, pub stage: u8, pub source: StageSelectionSource }
...
pub static DEST_ROUTE_HEALTH: OnceLock<DashMap<String, DashMap<String, RouteHealth>>> = OnceLock::new();
...
pub use state_and_startup::{start_socks5_server, RelayOptions, Socks5ServerGuard, WARNED_SOCKS4_LIMITATIONS};
pub use telemetry_bus::{init_telemetry_bus, send_telemetry, TelemetryEvent};

// --- UTILS ---
pub fn now_unix_secs() -> u64 { SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() }
...
pub fn is_bypassable_public_ip(ip: std::net::IpAddr) -> bool {
    match ip {
        ...
        std::net::IpAddr::V6(v6) => !v6.is_loopback() && !v6.is_unicast_link_local() && !v6.is_unique_local() && !v6.is_multicast() && !v6.is_unspecified(),
    }
}

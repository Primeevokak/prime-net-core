...
|--------------------------------|-------------------------------------------------|--------------------------------------------------|
| `disorder-shuffle-3`           | `--disorder 3 --drop-sack`                      | Перестановка первых 3 сегментов, блокировка SACK |
| `split-oob-1`                  | `--split 1 --oob 1`                             | Split + OOB-данные, дезориентируют DPI           |
| `discord-disorder-dropsack`    | `--disorder 1 --drop-sack`                      | Discord: разрыв SACK-сборки                      |
| `discord-disoob-dropsack`      | `--split 1 --disoob 1 --drop-sack`              | Discord: OOB при WebSocket Upgrade               |
| `discord-oob2-tlsrec-dropsack` | `--oob 2 --tlsrec 3+s --drop-sack --udp-fake 1` | Discord: комбо OOB + TLS record + UDP fake       |
| `ttl-3-fake`                   | `--fake 1 --ttl 3`                              | Fake-пакеты с низким TTL                         |
| `split-tls-record`             | `--split 3 --tlsrec 3+s`                        | Split + TLS record fragment                      |

**`--drop-sack`**: блокирует SACK-пакеты от сервера, что мешает ISP реассемблировать TCP-поток для инспекции.
**`--disoob`**: вставляет OOB-байты прямо в момент WebSocket Upgrade — именно там срабатывает инспекция Discord.
...
| DPI bypass      | 7+ профилей byedpi, disorder/split/OOB/drop-sack/TTL/fake-пакеты           |
